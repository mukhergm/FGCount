...
main(int argc, char *argv[])
{
	FILE *fp,*fr,*fw,*f1;
	  if (argc<4)
          ...
          printf("./logp_mr_calc_5.exe <bond order of each of the atoms in a molecule, outout of lp4.pl program> <Output file of lp2.exe> <Output file of lp3.exe> <Output file name>\n");
          exit(1);
          }
       	struct format
       {
	...
	if (fw==NULL)            
        {
        printf("logp_mr_calc_5.exe: Cannot write output. No output file name is given\n");
        exit(1);
        }	
	//int ln=0;
